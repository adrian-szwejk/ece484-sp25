...
class vehicleController():
    ...
    def __init__(self):
        ...
        # self.logx = []
        # self.logy = []
        # self.log_time = []  # Store time values
    ...
    def longititudal_controller(self, curr_x, curr_y, curr_vel, curr_yaw, future_unreached_waypoints):
        ...
        # Decay parameters
        lambda_factor = 5  # Controls the rate of decay
        curvature_threshold = 0.08  # Reference curvature for decay scaling

        # Find the closest waypoint within a reasonable lookahead distance
        target_point = None
        min_dist = float('inf')
        ...
        # If no point is found within lookahead distance, use the farthest waypoint
        if target_point is None:
            target_point = future_unreached_waypoints[-1]
            min_dist = math.sqrt((target_point[0] - curr_x) ** 2 + (target_point[1] - curr_y) ** 2)

        # Compute curvature using a simplified Pure Pursuit approach
        curvature = 0
        if min_dist > 0:
            curvature = 2 * (target_point[0] - curr_x) / (min_dist ** 2)

        # Apply exponential decay for velocity reduction based on curvature
        velocity_scale = math.exp(-lambda_factor * abs(curvature) / curvature_threshold)
        target_velocity = min_vel + (max_vel - min_vel) * velocity_scale  # Scaling velocity between min and max

        # Clamp velocity within bounds (just for safety)
        target_velocity = max(min_vel, min(target_velocity, max_vel))
    ...
    def stop(self):
        ...
        # plt.plot(self.log_time, self.log_accel)
